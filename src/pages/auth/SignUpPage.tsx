    ...
    return (
        <PageWrapper>
            ...
            <div className="flex justify-start items-start flex-col gap-3 rounded-md p-6 shadow-md text-white bg-zinc-900 sm:w-96 w-80">
                ...
                <InputField type="text" placeholder={t("username")}fieldError={!credentials.username && buttonPressed} 
                setState={(value) => setCredentials({...credentials, username: value})}/>

                <InputField type="text" placeholder={t("email")} fieldError={!credentials.email && buttonPressed} 
                setState={(value) => setCredentials({...credentials, email: value})}/>

                <InputField type="password" placeholder={t("password")} fieldError={!credentials.password && buttonPressed} 
                setState={(value) => setCredentials({...credentials, password: value})}/>

                <InputField type="password" placeholder={t("confirm-password")} fieldError={!credentials.confirmPassword && buttonPressed} 
